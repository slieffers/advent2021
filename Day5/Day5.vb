...
Public Structure Coord
    ...
    Public Shared Function GetNonDiagCoordsLine(coord1 As Coord, coord2 As Coord) As IEnumerable(Of Coord)
        ...
        If (coord1._x < coord2._x) Then
            line = (From x In Enumerable.Range(coord1._x, coord2._x - coord1._x + 1) Select CreateCoord(x, coord1._y)).ToList()
        Else If (coord2._x < coord1._x) Then
            line = (From x In Enumerable.Range(coord2._x, coord1._x - coord2._x + 1) Select CreateCoord(x, coord1._y)).ToList()
        Else If (coord1._y < coord2._y) Then
            line = (From y In Enumerable.Range(coord1._y, coord2._y - coord1._y + 1) Select CreateCoord(coord1._x, y)).ToList()
        Else If (coord2._y < coord1._y) Then
            line = (From y In Enumerable.Range(coord2._y, coord1._y - coord2._y + 1) Select CreateCoord(coord1._x, y)).ToList()
        End If
